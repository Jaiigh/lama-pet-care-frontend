...
import { CreateBookingPayload, getAvailableStaff, getStripeLink, type Staff } from "@/services/serviceService";
...
const BookPageInner = () => {
  ...
  const handlePayment = async () => {
    ...
    const startTime = selectedTimeSlots[0]; // First selected time slot
    const endTime = selectedTimeSlots[selectedTimeSlots.length - 1]; // Last selected time slot
    
    // Add 1 hour to end time
    const endTimePlusOne = (() => {
    ...
    const payload: CreateBookingPayload = {
      // Required for mservice, blank/undefined for cservice
      owner_id: localStorage.getItem("user_id") ?? undefined,
      payment_id: "", // Will be generated by backend
      pet_id: selectedPet,
      reserve_date_end: `${effectiveDate}T${endTimePlusOne}:00Z`,
      reserve_date_start: `${effectiveDate}T${startTime}:00Z`,
      service_type: selectedServiceType,
      staff_id: selectedStaff,
      status: "wait",
    };
    try {
      const stripeLink = await getStripeLink(payload);
      console.log("Redirecting to Stripe link:", stripeLink);
